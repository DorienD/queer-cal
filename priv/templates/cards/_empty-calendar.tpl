<div class="c-calendar__empty">
    {% if q.qhasobject or q.qongoing_on %}
        <p>
            {_ You filtered on _} {% if q.qhasobject %}<strong class="c-keyword-list__item">{{ q.qhasobject.title }}</strong>{% endif %}{% if q.qongoing_on %}{% if q.qhasobject %} and {% endif %}<strong class="c-keyword-list__item">{{ q.qongoing_on|date:"d-m-Y" }}</strong>{% endif %}. 
