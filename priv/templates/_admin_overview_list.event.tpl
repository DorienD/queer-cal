...
{% with "rsc.pivot_date_start" as field %}
...
<table class="table table-striped do_adminLinkedTable">
    <thead>
        <tr>
            ...
            <th>
                {% include "_admin_sort_header.tpl" field="rsc.pivot_date_start" caption=_"Date start" type="date" qsort=qsort %}
            </th>
            {# <th>
    ...
    {% for id in result|is_visible %}
        <tr class="{% if not id.is_published %}unpublished{% endif %}" data-href="{% url admin_edit_rsc id=id %}">
            ...
            <td>{{ id.date_start|date:_"d M Y, H:i" }}</td>
